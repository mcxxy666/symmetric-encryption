...
let unT = ETName (Pub, ef_zero)

let eplus (e1, e2) =
  if e1 = ef_zero then e2 else if e2 = ef_zero then e1 else Eplus (e1, e2)
...
let rec teq t1 t2 =
  if t1 = t2 || t1 = ETany || t2 = ETany then []
    ...
    match (t1, t2) with
    | ETName (_, e1), ETName (_, e1') -> [ ECeq (e1, e1') ]
...
let rec tpub ty =
  match ty with
  | ETUnit -> []
  | ETName (Pub, e1) -> [ ECsub (e1, Emap []) ]
  | ETName (Pr, e1) -> raise (PubWrong ty)
  | ETChan t -> raise (TypeError "tpub")
  | ETSKey t -> tpub t
  | ETEKey t -> raise (TypeError "unimp")
  | ETDKey t -> raise (TypeError "unimp")
...
let tgen ty =
  ...
  | ETName (Pub, e1) -> [ ECsub (e1, Emap []) ]
  | ETSKey _ -> []
  | _ -> raise (TypeError "tgen")

(* let rec tpub ty = *)
(*   let rec ttainted2 ty2 = *)
(*     match ty2 with *)
(*     | ETUnit -> [] *)
(*     | ETName (l, e1, e2) -> if l = Pub then [ ECsub (e2, Emap []) ] else [] *)
...
let shift_name n = match n with ENname _ -> n | ENindex i -> ENindex (i + 1)
let shift_names ns = List.map shift_name ns

let rec wfT ty ns =
  match ty with
  | ETUnit -> []
  | ETName (l, e1) -> [ ECfn (e1, ns) ]
  | ETSKey t -> wfT t ns
  | ETEKey t -> wfT t ns
  | ETDKey t -> wfT t ns
  | ETPair (t1, t2) ->
...
let rec sty2ety sty =
  match sty with
  | TUnit -> ETUnit
  | TName l -> ETName (l, Evar (fresh_evid ()))
  ...
  | TVar _ -> ETName (Pr, Evar (fresh_evid ()))

let stenv2etenv stenv =
  List.map
    (fun (n, st) ->
      match n with
      | ENname m -> (m, sty2ety st)
      | _ -> raise (Fatal "stenv2etenv"))
    stenv

let rec ef_subst e n ind = Esub (ind, n, e)

let rec et_subst t n ind =
  match t with
  | ETUnit -> ETUnit
  | ETName (l, e1) -> ETName (l, ef_subst e1 n ind)
...
let rec imitate_etype t =
  match t with
  | ETUnit -> ETUnit
  | ETName (l, e1) -> ETName (l, Evar (fresh_evid ()))
...
let plusT (t : etype) (e : effect) : etype * ef_constraint =
  match t with
  | ETName (l, e1) ->
      let ef_res = Evar (fresh_evid ()) in
      let c = [ ECeq (e1, Eplus (ef_res, e)) ] in
      (ETName (l, ef_res), c)
  | _ -> (t, [])
...
let rec msg_inf (env : et_env) (m : message) : effect * etype * ef_constraint =
  let ef, t, cc =
    ...
    | Name (ENname n) -> (
        let t1 =
          ...
          with Not_found -> raise (Fatal (n ^ " is undefined"))
        in
        match t1 with
        | ETName (l, ef2) ->
            ...
            let t = ETName (l, Eplus (ef2, ef2')) in
            let ce = [ ECeq (ef_zero, Eplus (ef_zero, ef3')) ] in
            (Eplus (ef3', ef2'), t, ce)
        (* | ETName _ -> *)
        (*     let ef_e = Evar (fresh_evid ()) in *)
        (*     let t_res, cc = plusT t1 ef_e in *)
        (*     (\* let ec3 = wfT t_res (dom env) in *\) *)
        (*     let ec3 = [] in *)
        (*     (ef_e, t_res, cc @ ec3) *)
        | _ -> (ef_zero, t1, []))
    ...
    | EncSym (m1, m2) -> (
        ...
        | ETSKey t0 ->
            ( eplus (ef1, ef2),
              unT,
              [ ECeq (ef2, Emap []) ] @ teq t0 t1 @ ec1 @ ec2 (* [] *) )
  ...
  in
  let _ =
    Printf.printf "\t[Message] %s ~%s |- %s:%s  (%s)\n" (Pprint.print_eenv env)
      (Pprint.print_effect ef) (Pprint.print_message m) (Pprint.print_etype t)
      (Pprint.print_ef_constraint cc)
  in
  (ef, t, cc)
...
let rec etype_inf (env : et_env) (p : stype procexp) =
  let ef, cc, p =
    ...
    | In (m, y, sty, pr) ->
        ...
        (ef, (* [], *)
             ec0 @ ec1 @ ec2 @ ec3, In (m, y, ty_n, pr'))
    | InS (m, y, sty, pr) ->
        let ef_m, ty_m, _ = msg_inf env m in
        let ty_n = sty2ety sty in
        let env' = env_extend y ty_n env in
    ...
    | Nu (x, sty, pr) -> (
        ...
        | ETName (l, e2) ->
            ...
            let () = Printf.printf "Nu:e2: %s" (Pprint.print_effect e2) in
            let ec2 =
              [
                ECsub (ef, Eplus (Evar ro, ef_check l (ENname x) e2));
                (* ECsub (e2, ef_zero); *)
                ECnotin (ENname x, Evar ro);
              ]
            in
    ...
    | Check (m, n, pr) -> (
        ...
        | ETName (l1, e2), ETName (l2, e2') ->
            if l1 = l2 then
              let ef_p, ec_p, pr' = etype_inf env pr in
              let ef = Emap [] in
              let ro = fresh_evid () in
              (*** eo in Chk ***)
              let r1 = fresh_evid () in
              (*** e ***)
              let ec_e =
                [
                  (* ECeq (ef_zero, ef); *)
              ...
              in
              let () =
                Printf.printf "Nu:(Evar ro): %s" (Pprint.print_effect (Evar ro))
              in
              ( Eplus (Evar r1, ef_check l1 em (Evar ro)),
                (* [], *)
                ec_p @ ec_m @ ec_n @ ec_e,
                Check (m, n, pr') )
    ...
    | DecSym (m, y, sty, k, pr) ->
        ...
        let ec_e =
          [
            ECsub (ef_m, ef_zero); ECeq (ef_k, ef_zero); ECnotin (ENname y, ef);
          ]
        in
        ( ef,
          (* [], *)
          ec @ ec_m @ ec_k @ ec_t @ ec_e,
          DecSym (m, y, ty_y, k, pr') )
    ...
    | End m -> (ef_end m, [], End m)
  in
  let _ =
    Printf.printf "[Proc] %s ~ %s\n|-" (Pprint.print_eenv env)
      (Pprint.print_effect ef);
    Pprint.pp_eproc p;
    Printf.printf "\n\t%s\n" (Pprint.print_ef_constraint cc)
  in
  (ef, cc, p)
...
let einf stenv p =
  ...
  let ec2 = List.flatten (List.map (fun (x, t) -> tgen t) etenv) in
  let ec3 = [ ECsub (ef, ef_zero) ] in
  let res = ec3 @ ec2 @ ec1 in
  let _ = Printf.printf "print_ef_constraint:\n" in
  let _ =
    print_string
      ("# of effect constr.: " ^ string_of_int (List.length res) ^ "\n")
  in
  let _ = Printf.printf "%s\n" (Pprint.print_ef_constraint res) in
  (res, p1)
